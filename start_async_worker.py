...
from busy_beaver.config import IN_PRODUCTION, config
from busy_beaver.extensions import rq
from busy_beaver.tasks.handlers import retry_failed_job

# observability
config.LOGGING_CONFIG["handlers"]["datadog_file"]["filename"] = "logs/busy_beaver_worker.json"
load_dict_config(config["LOGGING_CONFIG"])
if IN_PRODUCTION and config.SENTRY_DSN:
    sentry_sdk.init(config.SENTRY_DSN, integrations=[RqIntegration()])
